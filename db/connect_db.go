...
func CreateMysqlPool(host, user, password, dbname string, port int) *gorm.DB {
	...
	if err != nil {
		utils.LogRus.Panicf("%s连接数据库失败:%s", dsn, err)
	}
...
func ConnectMySQL() *gorm.DB {
	blogMySQLOnce.Do(func() { //保证在高并发多协程情况下if代码块只被执行一次，确保连接池的单例模式
		if blogMySQL == nil { //如果该值为空则创建连接池
			mysqlViper := utils.ReadConfig("mysql")
			host := mysqlViper.GetString("mysql.host")
			user := mysqlViper.GetString("mysql.user")
			password := mysqlViper.GetString("mysql.password")
			dbname := mysqlViper.GetString("mysql.dbname")
			port := mysqlViper.GetInt("mysql.port")
